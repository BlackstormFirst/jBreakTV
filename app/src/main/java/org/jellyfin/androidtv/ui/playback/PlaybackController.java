...
public class PlaybackController implements PlaybackControllerNotifiable {
    ...
    private Lazy<PlaybackIndexManager> playbackIndexManager = inject(PlaybackIndexManager.class);
    ...
    private void setDefaultVideoIndex(StreamInfo info) {
        if (mDefaultVideoIndex == -1) {
            Integer bestIndex = playbackIndexManager.getValue().getBestVideoIndex(info.getMediaSource());
    ...
    private void setDefaultAudioIndex(StreamInfo info) {
        if (mDefaultAudioIndex == -1) {
            Integer lastChosenLanguage = playbackIndexManager.getValue().getBestAudioIndex(info.getMediaSource());
